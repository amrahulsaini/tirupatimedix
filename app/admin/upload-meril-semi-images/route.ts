...
export async function POST(request: Request) {
  try {
    ...
    const semiIdRaw = formData.get("meril_semi_id");
    const semiId = Number(semiIdRaw);

    if (!Number.isInteger(semiId) || semiId <= 0) {
    ...
    const [semiRows] = await dbQuery<Array<{ id: number }>>(
      "SELECT id FROM meril_semi_automatic WHERE id = ? LIMIT 1",
      [semiId]
    );
    if (!semiRows.length) {
      return respondUpload(request, "invalid");
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "meril-semi", String(semiId));
    await mkdir(uploadDir, { recursive: true });

    const [sortRows] = await dbQuery<Array<{ max_sort: number | null }>>(
      "SELECT COALESCE(MAX(sort_order), 0) AS max_sort FROM meril_semi_product_images WHERE meril_semi_product_id = ?",
      [semiId]
    );
    ...
    for (const file of files) {
      ...
      const publicPath = `/uploads/meril-semi/${semiId}/${fileName}`;

      const bytes = await file.arrayBuffer();
      await writeFile(diskPath, Buffer.from(bytes));

      await dbQuery(
        "INSERT INTO meril_semi_product_images (meril_semi_product_id, image_path, sort_order) VALUES (?, ?, ?)",
        [semiId, publicPath, nextSort]
      );
    }

    return respondUpload(request, "ok");
  } catch (error) {
    console.error("Failed to upload Meril Semi-Auto product images", error);
