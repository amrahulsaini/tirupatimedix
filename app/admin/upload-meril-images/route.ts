...
export async function POST(request: Request) {
  try {
    ...
    const merilIdRaw = formData.get("meril_id");
    const merilId = Number(merilIdRaw);

    if (!Number.isInteger(merilId) || merilId <= 0) {
    ...
    const [merilRows] = await dbQuery<Array<{ id: number }>>(
      "SELECT id FROM meril_fully_automatic WHERE id = ? LIMIT 1",
      [merilId]
    );
    if (!merilRows.length) {
      return respondUpload(request, "invalid");
    ...
    const uploadDir = path.join(process.cwd(), "public", "uploads", "meril", String(merilId));
    await mkdir(uploadDir, { recursive: true });

    const [sortRows] = await dbQuery<Array<{ max_sort: number | null }>>(
      "SELECT COALESCE(MAX(sort_order), 0) AS max_sort FROM meril_product_images WHERE meril_product_id = ?",
      [merilId]
    );
    ...
    for (const file of files) {
      ...
      const publicPath = `/uploads/meril/${merilId}/${fileName}`;

      const bytes = await file.arrayBuffer();
      await writeFile(diskPath, Buffer.from(bytes));

      await dbQuery(
        "INSERT INTO meril_product_images (meril_product_id, image_path, sort_order) VALUES (?, ?, ?)",
        [merilId, publicPath, nextSort]
      );
    }

    return respondUpload(request, "ok");
  } catch (error) {
    console.error("Failed to upload Meril product images", error);
